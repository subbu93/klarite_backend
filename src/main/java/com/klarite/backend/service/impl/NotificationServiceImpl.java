...
public class NotificationServiceImpl implements NotificationService {
    ...
    public List<Notification> get(Long userId, Boolean getActive, JdbcTemplate jdbcTemplate) {
        ...
        try {
            ...
            if (Privileges.skillObserver.contains(usr.getRole())) {
                ...
                query = query.replace("%1", usr.getBusinessUnitId().toString()).replace("%2", usr.getCostCenterId().toString());
            ...
            for (Map<String, Object> row : rows) {
                ...
                switch(type) {
                    case NotificationType.ObservationRequest: notifications.add(getRequestedObservations(row, usr, jdbcTemplate)); break;
                    case NotificationType.ObservationResponse: notifications.add(getObservationResponse(row, usr, jdbcTemplate)); break;
                    case NotificationType.UpcomingTraining: notifications.add(getUpcomingTrainings(row, usr, jdbcTemplate)); break;
                    case NotificationType.SkillValidation: notifications.add(getValidationNotification(row, usr, jdbcTemplate)); break;
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private Notification getUpcomingTrainings(Map<String, Object> row, User usr, JdbcTemplate jdbcTemplate) {
        ...
        obj.init((Long) row.get("id"), (Boolean) row.get("is_active"), usr.getId(), usr.fetchFullName(), 
            null, null, (String) row.get("payload"));
        
        TrainingAssignment assignment = trainingService.getAssignmentDetails(obj.getTrainingAssignmentId(), jdbcTemplate);
    ...
    private Notification getObservationResponse(Map<String, Object> row, User usr, JdbcTemplate jdbcTemplate)  {
        ...
        obj.init((Long) row.get("id"), (Boolean) row.get("is_active"), (Long) row.get("receiver_id"), usr.fetchFullName(), senderId, 
            sender.fetchFullName(), (String) row.get("payload"));
        return obj;
    }
